...
pub fn LengthPrefixedField(comptime Prefix: type, comptime endian: std.builtin.Endian) type {
    return struct {
        ...
        pub fn read(reader: *bounded.BoundedReader) Error!@This() {
            ...
            const prefix_bytes = reader.readBytes(@sizeOf(Prefix)) catch return error.UnexpectedEnd;
            var fixed: [@sizeOf(Prefix)]u8 = undefined;
            @memcpy(&fixed, prefix_bytes);
        ...
        pub fn subReader(self: @This()) bounded.BoundedReader {
            return bounded.BoundedReader.init(self.payload);
        }
    };
}

test "reads borrowed field and rolls back truncation" {
    var r = bounded.BoundedReader.init(&.{ 0, 2, 9, 8 });
    const f = try LengthPrefixedField(u16, .big).read(&r);
    try std.testing.expectEqualSlices(u8, &.{ 9, 8 }, f.bytes());
    var bad = bounded.BoundedReader.init(&.{ 3, 1 });
    try std.testing.expectError(error.UnexpectedEnd, LengthPrefixedField(u8, .little).read(&bad));
    try std.testing.expectEqual(@as(usize, 0), bad.position());
}
test "zero payload is supported" {
    var r = bounded.BoundedReader.init(&.{0});
    const f = try LengthPrefixedField(u8, .little).read(&r);
    try std.testing.expectEqual(@as(usize, 0), f.bytes().len);
}
