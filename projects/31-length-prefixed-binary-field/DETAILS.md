# Length-Prefixed Binary Field Details
...
## Public surface
All `pub` declarations in `projects/31-length-prefixed-binary-field/src/length_prefixed_binary_field.zig` are supported. Exact signatures, structured inputs, outputs, errors, mutation, borrowing, and endpoint guarantees are indexed in `details.json`.

## Import and location
Named import: `length-prefixed-binary-field`. Implementation: `projects/31-length-prefixed-binary-field/src/length_prefixed_binary_field.zig`. External test: `projects/31-length-prefixed-binary-field/tests/smoke_test.zig`.
...
## Dependencies
`bounded-byte-reader`, `checked-integer-cast`, `endian-integer-codec`, `binary-cursor-checkpoint`, `bounded-binary-sub-reader`. Exact paths and imported symbols are recorded in `details.json`.
...
```zig
const component = @import("length-prefixed-binary-field");
...
## Validation
Unit: `zig build test-length-prefixed-binary-field`. Smoke: `zig build smoke-length-prefixed-binary-field`. Contract formatting/checking completed; Zig compiler execution is pending because Zig is unavailable.

## Source map
Implementation, unit tests: `projects/31-length-prefixed-binary-field/src/length_prefixed_binary_field.zig`. Smoke: `projects/31-length-prefixed-binary-field/tests/smoke_test.zig`. Documentation: this file, `README.md`, `MASTERY.md`, and `details.json`. No examples, fixtures, benchmarks, or fuzz targets.
