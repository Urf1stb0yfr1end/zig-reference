# ELF64 Program-Header Parser Mastery

## Mental model and representation
Parse validated ELF64 segments and bounded fixed-capacity tables. Treat each value as proof that its documented invariants were checked. Logical state is separate from backing bytes or inline capacity.
...
Read `fixed-capacity-vector`, `bounded-byte-reader`, `checked-integer-cast`, `validated-enum-decoder`, `aligned-address-and-size-helpers`, `validated-bit-flags`, `checked-half-open-range`, `distinct-memory-address-types`, `endian-integer-codec`, `binary-cursor-checkpoint`, `bounded-binary-sub-reader`, `elf64-file-header-parser` contracts first. Adapt capacity, endian, advancement, classification, and unknown-value policy without weakening lower guarantees.
