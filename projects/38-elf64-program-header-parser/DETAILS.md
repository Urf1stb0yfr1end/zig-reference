...
`PT_GNU_EH_FRAME`, `PT_GNU_STACK`, `PT_GNU_RELRO`, and `PT_RISCV_ATTRIBUTES` have stable typed identities rather than falling into the unknown-type policy. The ELF `p_filesz <= p_memsz` rule is enforced for `PT_LOAD`; file-only metadata segments may truthfully have no memory image.
