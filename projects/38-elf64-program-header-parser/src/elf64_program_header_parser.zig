...
pub const Permission = enum(u32) { execute = 1, write = 2, read = 4 };
pub const Permissions = flag_module.ValidatedBitFlags(Permission);
...
fn read(comptime T: type, r: *bounded.BoundedReader, e: elf.ElfEndian) error{UnexpectedEnd}!T {
...
pub fn parseOne(reader: *bounded.BoundedReader, endian: elf.ElfEndian) ParseError!Segment {
    _ = enums;
    const mark = checkpoints.capture(reader.*);
    errdefer mark.restore(reader) catch unreachable;
    const raw_type = try read(u32, reader, endian);
    const raw_flags = try read(u32, reader, endian);
    const offset = try read(u64, reader, endian);
    const virtual = try read(u64, reader, endian);
    const physical = try read(u64, reader, endian);
    ...
    const v = cast.checkedIntegerCast(usize, virtual) catch return error.OutOfRange;
    const ms = cast.checkedIntegerCast(usize, memory_size) catch return error.OutOfRange;
    const p = cast.checkedIntegerCast(usize, physical) catch return error.OutOfRange;
    const st: SegmentType = switch (raw_type) {
        0 => .{ .known = .null },
        1 => .{ .known = .load },
        2 => .{ .known = .dynamic },
        3 => .{ .known = .interpreter },
...
pub fn parseTable(comptime capacity_: usize, file_reader: *bounded.BoundedReader, header: elf.Elf64FileHeader) ParseError!vectors.FixedVector(Segment, capacity_) {
    if (header.program_entry_size != 56) return error.InvalidTableEntrySize;
    const table = header.programTableRange(file_reader.extent()) catch |e| switch (e) {
    ...
    var i: usize = 0;
    while (i < header.program_count) : (i += 1) result.append(try parseOne(region.reader(), header.endian)) catch return error.Full;
    return result;
}
test "validates one segment and rejects filesz greater than memsz" {
    var bytes = [_]u8{0} ** 56;
    bytes[0] = 1;
    bytes[4] = 4;
    bytes[32] = 2;
    ...
    try std.testing.expectError(error.FileSizeExceedsMemorySize, parseOne(&r, .little));
