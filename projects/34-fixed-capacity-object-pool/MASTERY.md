# Fixed-Capacity Object Pool Mastery

## Mental model and representation
Expose the existing generational table as an iterable, resettable object-pool contract. Treat each value as proof that its documented invariants were checked. Logical state is separate from backing bytes or inline capacity.
...
Read `bitmap-allocator`, `generational-handles` contracts first. Adapt capacity, endian, advancement, classification, and unknown-value policy without weakening lower guarantees.
