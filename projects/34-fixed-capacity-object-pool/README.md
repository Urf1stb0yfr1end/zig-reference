...
## Design and use
Import `@import("fixed-capacity-object-pool")`; the public declarations in `src/fixed_capacity_object_pool.zig` provide the complete API. Borrowed views never copy input, fixed-capacity modules allocate nothing, and owned storage requires explicit cleanup.

## Dependencies and inherited guarantees
`bitmap-allocator`, `generational-handles`. Their checked bounds, casts, address domains, allocation state, and stale-handle rules are inherited rather than recreated.
