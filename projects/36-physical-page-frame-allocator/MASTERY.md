# Physical Page-Frame Allocator Mastery

## Mental model and representation
Allocate physical frames from a validated usable region with fixed metadata. Treat each value as proof that its documented invariants were checked. Logical state is separate from backing bytes or inline capacity.
...
Read `bit-set`, `bitmap-allocator`, `checked-half-open-range`, `distinct-memory-address-types`, `physical-page-frame-number-and-address-conversion`, `physical-memory-region-set` contracts first. Adapt capacity, endian, advancement, classification, and unknown-value policy without weakening lower guarantees.
