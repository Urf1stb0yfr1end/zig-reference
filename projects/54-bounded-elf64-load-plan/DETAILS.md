...
## Contract
`plan(capacity, bytes)` reuses `elf64-file-header-parser.parse` and `elf64-program-header-parser.parseTable`. It accepts ELF64, little endian, version 1, `ET_EXEC`, machine 243, at most 64 program headers, and at least one `PT_LOAD`. Dynamic, interpreter, TLS, shared-library, and unknown rows are rejected; null, note, program-header, GNU-stack, GNU-RELRO, and RISC-V-attributes metadata rows are ignored.
