...
Use `MaterializedImage(page_capacity).prepare(bytes, load_plan, bias)` during PREPARE to produce an allocation-free owned set of zero-initialized pages. It iterates every planned segment, handles unaligned first and final pages, copies exact file bytes, leaves BSS and untouched bytes zero, merges shared-page permissions without permitting W+X, and fails explicitly on address overflow, source drift, or page capacity. The caller reserves physical backing, copies these prepared pages through supervisor access, and installs only their final permissions during COMMIT.
