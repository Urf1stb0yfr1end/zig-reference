# Mastering Fixed Capacity Topological Sort

## Mental model

Produce a deterministic dependency order or report cycles in a bounded graph. The representation has a fixed bound, performs no hidden allocation, owns only inline metadata, and is single-threaded unless the caller synchronizes all access.

## Invariants and reasoning

Successful operations preserve representation bounds. Errors `TooManyNodes, InvalidNode, DuplicateEdge, Full, Cycle` leave state unchanged. Returned values are copied unless the API returns a documented pointer or slice; such borrows end when their backing caller-owned storage ends and are invalidated by the mutations named in `DETAILS.md`.
