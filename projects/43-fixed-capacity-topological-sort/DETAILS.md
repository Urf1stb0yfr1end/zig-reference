# Fixed Capacity Topological Sort integration contract

## Purpose

Produce a deterministic dependency order or report cycles in a bounded graph.
...
The public root symbol is `FixedTopologicalGraph`. Its constructors and methods are defined in `src/fixed_capacity_topological_sort.zig`; all mutations are bounded, allocation-free, and report `TooManyNodes, InvalidNode, DuplicateEdge, Full, Cycle` where applicable.

## Import and location

Import `fixed-capacity-topological-sort`. Implementation and unit tests: `projects/43-fixed-capacity-topological-sort/src/fixed_capacity_topological_sort.zig`. External smoke test: `projects/43-fixed-capacity-topological-sort/tests/smoke_test.zig`.
...
`TooManyNodes, InvalidNode, DuplicateEdge, Full, Cycle` are explicit. Every reported failure is failure-atomic and leaves state unchanged. Empty optional queries also leave state unchanged.
...
- `fixed-capacity-priority-queue`: imported through its named module and inherits its bounded/failure semantics.
...
```zig
const foundation = @import("fixed-capacity-topological-sort");
_ = foundation.FixedTopologicalGraph;
```
...
Run `zig build test-fixed-capacity-topological-sort` and `zig build smoke-fixed-capacity-topological-sort` under Zig 0.14.0. Tests cover success, empty/zero capacity, full/bounds, overflow or invalid state where applicable, and failure atomicity.
