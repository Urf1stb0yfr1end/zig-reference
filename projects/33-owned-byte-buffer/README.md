...
## Design and use
Import `@import("owned-byte-buffer")`; the public declarations in `src/owned_byte_buffer.zig` provide the complete API. Borrowed views never copy input, fixed-capacity modules allocate nothing, and owned storage requires explicit cleanup.

## Dependencies and inherited guarantees
`dynamic-array`, `byte-writer`. Their checked bounds, casts, address domains, allocation state, and stale-handle rules are inherited rather than recreated.
