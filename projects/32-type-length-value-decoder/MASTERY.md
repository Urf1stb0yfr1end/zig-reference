# Type-Length-Value Decoder Mastery

## Mental model and representation
Iterate allocation-free TLV records while preserving unknown tags. Treat each value as proof that its documented invariants were checked. Logical state is separate from backing bytes or inline capacity.
...
Read `bounded-byte-reader`, `checked-integer-cast`, `endian-integer-codec`, `binary-cursor-checkpoint`, `bounded-binary-sub-reader`, `length-prefixed-binary-field` contracts first. Adapt capacity, endian, advancement, classification, and unknown-value policy without weakening lower guarantees.
