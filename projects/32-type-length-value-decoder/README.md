...
## Design and use
Import `@import("type-length-value-decoder")`; the public declarations in `src/type_length_value_decoder.zig` provide the complete API. Borrowed views never copy input, fixed-capacity modules allocate nothing, and owned storage requires explicit cleanup.

## Dependencies and inherited guarantees
`bounded-byte-reader`, `checked-integer-cast`, `endian-integer-codec`, `binary-cursor-checkpoint`, `bounded-binary-sub-reader`, `length-prefixed-binary-field`. Their checked bounds, casts, address domains, allocation state, and stale-handle rules are inherited rather than recreated.
