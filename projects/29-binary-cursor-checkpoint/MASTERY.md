# Binary Cursor Checkpoint Mastery

## Mental model and representation
Capture and restore bounded-reader positions for speculative parsing. Treat each value as proof that its documented invariants were checked. Logical state is separate from backing bytes or inline capacity.
...
Read `bounded-byte-reader` contracts first. Adapt capacity, endian, advancement, classification, and unknown-value policy without weakening lower guarantees.
