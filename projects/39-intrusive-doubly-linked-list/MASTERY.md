# Mastering Intrusive Doubly Linked List

## Mental model

Maintain caller-owned nodes in an allocation-free doubly linked list. The representation has a fixed bound, performs no hidden allocation, owns only inline metadata, and is single-threaded unless the caller synchronizes all access.

## Invariants and reasoning

Successful operations preserve representation bounds. Errors `AlreadyLinked` leave state unchanged. Returned values are copied unless the API returns a documented pointer or slice; such borrows end when their backing caller-owned storage ends and are invalidated by the mutations named in `DETAILS.md`.
