# Mastering Fixed Bump Allocator

## Mental model

Allocate aligned slices monotonically from a caller-owned byte region. The representation has a fixed bound, performs no hidden allocation, owns only inline metadata, and is single-threaded unless the caller synchronizes all access.

## Invariants and reasoning

Successful operations preserve representation bounds. Errors `InvalidAlignment, Overflow, OutOfMemory` leave state unchanged. Returned values are copied unless the API returns a documented pointer or slice; such borrows end when their backing caller-owned storage ends and are invalidated by the mutations named in `DETAILS.md`.
