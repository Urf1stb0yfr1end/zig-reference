# Physical Memory Region Set Details
...
## Public surface
All `pub` declarations in `projects/35-physical-memory-region-set/src/physical_memory_region_set.zig` are supported. Exact signatures, structured inputs, outputs, errors, mutation, borrowing, and endpoint guarantees are indexed in `details.json`.

## Import and location
Named import: `physical-memory-region-set`. Implementation: `projects/35-physical-memory-region-set/src/physical_memory_region_set.zig`. External test: `projects/35-physical-memory-region-set/tests/smoke_test.zig`.
...
## Dependencies
`fixed-capacity-vector`, `validated-enum-decoder`, `checked-half-open-range`, `distinct-memory-address-types`, `physical-page-frame-number-and-address-conversion`. Exact paths and imported symbols are recorded in `details.json`.
...
```zig
const component = @import("physical-memory-region-set");
...
## Validation
Unit: `zig build test-physical-memory-region-set`. Smoke: `zig build smoke-physical-memory-region-set`. Contract formatting/checking completed; Zig compiler execution is pending because Zig is unavailable.

## Source map
Implementation, unit tests: `projects/35-physical-memory-region-set/src/physical_memory_region_set.zig`. Smoke: `projects/35-physical-memory-region-set/tests/smoke_test.zig`. Documentation: this file, `README.md`, `MASTERY.md`, and `details.json`. No examples, fixtures, benchmarks, or fuzz targets.
