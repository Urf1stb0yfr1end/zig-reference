# Bounded Binary Sub-Reader Mastery

## Mental model and representation
Confine a child reader and make parent advancement policy explicit. Treat each value as proof that its documented invariants were checked. Logical state is separate from backing bytes or inline capacity.
...
Read `bounded-byte-reader`, `binary-cursor-checkpoint` contracts first. Adapt capacity, endian, advancement, classification, and unknown-value policy without weakening lower guarantees.
