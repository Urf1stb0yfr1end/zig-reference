...
`python3 tools/pressure-real-rv64-userspace.py --artifact-only --output-dir PATH` copies both hash-verified executable artifacts to explicit caller-owned temporary/generated storage for a downstream machine build. `zig build install-freestanding-riscv64-morphic-runtime -Dexternal-rv64-artifact=PATH --prefix PREFIX` embeds those exact bytes through host-side build transport without adding artifact transport to Morphic semantics. `python3 tools/verify-freestanding-riscv64-external-artifact-transport.py --self-test` verifies one exact occurrence in the freestanding ELF and causally rejects mutated identity; omit `--self-test` for the ordinary exact-identity transport proof. These commands do not claim U-mode execution.
