...
Omitting `-Dexternal-rv64-argv1` selects the live SBI legacy console input backend for a namespace-backed external process: `zig build install-freestanding-riscv64-morphic-runtime -Dexternal-rv64-namespace-manifest=PATH/namespace.json -Dexternal-rv64-namespace-data=PATH/namespace.data -Dexternal-rv64-argv0=/bin/sh --prefix PATH`. Run the resulting machine interactively with `qemu-system-riscv64 -machine virt -nographic -bios default -kernel PATH/bin/morphic-freestanding-riscv64`. Supplying `argv1` preserves deterministic fixture input. Batch 32D compiled the machine and unit-tested resource-owned backend state under Zig 0.14.0; the interactive retry remained not run because `qemu-system-riscv64` was unavailable in that environment.
