...
- `FUTURE_RESEARCHER_FREEDOM_PRINCIPLE.md` — preserves the explicit rule that Morphic must prevent future researchers from becoming trapped in QuirkM's own accumulated baggage while still allowing them to inherit proven mechanisms, compatibility knowledge, and software civilization.
...
- `QART_ARCHITECTURE_AND_PORTABILITY_PRINCIPLES.md` — establishes QArt's foundation-first creative architecture: explicit document/layer, nondestructive operation, renderer/cache, and transaction-history models; a portable core that can be developed on Linux while QuirkM matures; and native QuirkM integrations for creator-focused resource sovereignty.
