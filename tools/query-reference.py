...
def agent_query(argv):
    ...
    if kind == "doctor":
        ...
        for component,cmd in [("agent index drift",[sys.executable,"tools/build-agent-index.py","--check"]),("Python validation environment",[sys.executable,"tools/python-environment.py","--check"]),("agent contracts",[sys.executable,"tools/python-environment.py","tools/validate-agent-contracts.py","--quick"])]:
            run=subprocess.run(cmd,cwd=ROOT,capture_output=True,text=True,timeout=30); ok=run.returncode==0; bad|=not ok
            checks.append({"component":component,"status":"ok" if ok else "error","reason":"check passed" if ok else (run.stderr.strip().splitlines()[0] if run.stderr.strip() else "check failed"),"repair":"python3 tools/build-agent-index.py" if "index" in component and not ok else "python3 -m venv .venv && .venv/bin/python -m pip install -r tools/requirements.txt" if "Python" in component or "contracts" in component else ""})
