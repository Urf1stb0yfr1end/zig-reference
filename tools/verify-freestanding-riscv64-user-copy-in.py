...
def parse(raw,truth):
 ...
 if [n['first_sepc'],n['prepared_sepc'],n['second_sepc']]!=pcs: raise RuntimeError("ELF-derived service/return/terminal PC contradiction")
 ...
 if len(rows)!=n['final_leaf_count'] or n['final_u_leaves']!=2 or n['final_wx_leaves']!=0: raise RuntimeError("final leaf count/U/W+X contradiction")
 def leaf_map(values):
  result={}
  for row in values:
   if set(row)!={'leaf_va','pa','pte','level'}: raise RuntimeError("malformed final leaf row")
   item=tuple(hx(row,k) for k in ('leaf_va','pa','pte','level'))
   if item[0] in result: raise RuntimeError("duplicate final leaf VA")
   result[item[0]]=item[1:]
  return result
 if leaf_map(rows)!=leaf_map(old_rows): raise RuntimeError("Batch 21B final leaf truth differs from Batch 20")
 users=sum(bool(int(row['pte'],16)&0x10) for row in rows); wx=sum((int(row['pte'],16)&0xc)==0xc for row in rows)
 if users!=2 or wx!=0 or (users,wx)!=(n['final_u_leaves'],n['final_wx_leaves']): raise RuntimeError("raw final leaves do not prove U=2/W+X=0")
...
def self_test():
 with tempfile.TemporaryDirectory() as p:
  ...
  muts += [
   (f"prepared_sepc={int(d['prepared_sepc'],16):016x}".encode(), b"prepared_sepc=0000000080401000"),
   (b"final_u_leaves=0000000000000002", b"final_u_leaves=0000000000000003"),
   (b"final_wx_leaves=0000000000000000", b"final_wx_leaves=0000000000000001"),
   (f"final_leaf_count={int(d['final_leaf_count'],16):016x}".encode(), f"final_leaf_count={int(d['final_leaf_count'],16)+1:016x}".encode()),
  ]
  _, leaf_rows=prior.parse_fields(raw,BEGIN,END)
  first=("leaf_va="+leaf_rows[0]['leaf_va']+",pa="+leaf_rows[0]['pa']+",pte="+leaf_rows[0]['pte']+",level="+leaf_rows[0]['level']).encode()
  muts += [(first,b""),(first,first.replace(b"pte=",b"pte=000000000000000f,",1)),(first,first+first)]
  code=next(("leaf_va="+r['leaf_va']+",pa="+r['pa']+",pte="+r['pte']+",level="+r['level']).encode() for r in leaf_rows if r['leaf_va']=='0000000080401000')
  muts += [(code,code.replace(b"5b,level",b"5f,level")),(code,code+code.replace(b"80401000",b"80403000",1))]
