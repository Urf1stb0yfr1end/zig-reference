...
def build():
    ...
    for d in cs:
        ...
        mods.append({"id":m["id"],"name":n,"display_name":m["display_name"],"summary":m["summary"],"directory":m["directory"],"status":m["status"],"declared_maturity":m["maturity"],"details":d["locations"]["details_json"],"source":d["locations"]["public_entrypoint"]})
    ...
    for m in mods:
        ...
        status.append({"module":m["name"],"lifecycle":"active" if m["status"].startswith("implemented") else "experimental","maturity_level":level,"evidence_records":len(ev)})
    ontology=[]
